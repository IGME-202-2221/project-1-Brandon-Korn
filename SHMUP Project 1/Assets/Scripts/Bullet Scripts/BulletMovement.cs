...
public class BulletMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            velocity = allBullets[i].GetComponent<BulletInfo>().direction * allBullets[i].GetComponent<BulletInfo>().speed * Time.deltaTime;
    ...
    public void createBullet(Transform ship, Vector3 bulletDirection, bulletType type)
    {
       ...
       newBullet.transform.position = ship.position;
        ...
        if (type == bulletType.spin ||
            type == bulletType.player)
        {
            newBullet.GetComponent<BulletInfo>().speed = 8;
        }
        else if (type == bulletType.turret)
        {
            newBullet.GetComponent<BulletInfo>().speed = 5.5f;
        }
